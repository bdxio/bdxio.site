...
const filteredTalks = computed(() => {
  ...
  if (currentFilter.value === ALL_CATEGORIES) return talks.value;
...
<template>
  <main class="talks-page">
    <Heading level="1" class="talks-title">
      Les talks de {{ year }} <span v-if="filteredTalks.length" class="talks-count">({{ filteredTalks.length }})</span>
    </Heading>
    ...
    <div class="talks-list-wrapper">
      <ul class="talks-list">
        <li v-for="talk in filteredTalks" :key="`talk-${talk.id}`">
          <SectionTalkCard :talk="talk" :to="`/${year}/talks/${talk.id}`" />
        </li>
      </ul>
    </div>
  </main>
