...
class SomeSpeakersPresentation extends Component {
  ...
  getRandomSpeaker(ref, speakers) {
    ...
    return isSelected || randomSpeaker["Nom"] === ""
      ? this.getRandomSpeaker(ref, speakers)
      : randomSpeaker;
  }
  ...
  getRandomSpeakers(speakers) {
    ...
    if (speakers.length > 0) {
      const LIMIT =
        speakers.length >= COUNT_DISPLAYED_SPEAKERS ? COUNT_DISPLAYED_SPEAKERS : speakers.length;
    ...
    return (
      <div className="row users text-center">
        ...
        <div className="column small-12">
          ...
          <h6>
            Quelques profils{" "}
            <strong className="red">{isPrevious ? "de l'édition 2018" : ""} </strong> choisis de
            façon aléatoire pour vous mettre l'eau à la bouche.
          </h6>
        </div>
          ...
          <div className="column small-12 medium-8">
            <div className="row align-center users-row-1 users-container-content">
              {speakers.slice(0, 2).map((speaker, i) => (
                <UserCard key={`speaker_${i}`} user={speaker} noHover/>
              ))}
            </div>
          </div>
          ...
          <div className="column small-12 medium-8">
            <div className="row align-center users-row-1 users-container-content">
              {speakers.slice(-2).map((speaker, i) => (
                <UserCard key={`speaker_${i}`} user={speaker} imageAtRight={!isMobile} noHover />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="align-center users-button">
          <Link
            className="shrink text-center button small secondary"
            to={isPrevious ? "/2018/speakers" : "/speakers"}
          >
            Les voir tous
          </Link>
        </div>
